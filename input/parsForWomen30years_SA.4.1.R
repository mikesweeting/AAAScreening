...
################################################################################
# Parameters for phase II (women). 
#@ parsForWomen30years.R was previously called parametersForWomen.R
...
# "SWAN: Parameters and sources of information for modelling population 
# AAA screening in women - version 2"

################################################################################
# NUMBERS THAT DO NOT APPEAR IN THE WORD DOCUMENT BUT SHOULD DO
...
v0$generateCensoringTime <- function() { 25.000001 }  # see parsForMen30years.R


# Post-surgery monitoring. 
# Set the parameters so that these never happen. 
...
v2$probOfAttendScreen <- setType(2028/2999, "probability")
v1distributions$probOfAttendScreen <- 
  setType(list(alpha=2028, beta=2999-2028), "beta pars for probability")
...
v2$prevalence <- setType(0.0070, "probability")
v1distributions$prevalence <- 
  setType(list(mean=-4.954821 , variance=(0.32321)^2), "normal distribution for logit prevalence")
...
# Values for rupture (v2$gamma and v$2alpha) taken from table 4.10 from ch4_191216, 
#     emailed to KM by MS on 11/01/17.
...
v1other$aortaDiameterThresholds <- c(2.5, 3.0, 4.0, 5.0)
v1other$monitoringIntervals <- c(5, 1, 0.25)
...
# (Not in Word doc) Model for peri/post-operative mortality
...
## REMOVED: Discussions between MS, ST and KM June 2017
# Non-AAA mortality rate in those contraindicated
#v2$rateOfNonAaaDeathAfterContraindication <- setType(23.96 / 100, "rate")
#v1distributions$rateOfNonAaaDeathAfterContraindication <- 
#  setType(list(shape=41, scale=1/171.1), "gamma pars for rate")
...
# QoL harms of screening
# QoL harms of surgery
# These will need new programming, in calculateHealthEconomicQuantities, 
